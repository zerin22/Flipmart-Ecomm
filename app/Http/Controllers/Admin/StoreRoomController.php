...
class StoreRoomController extends Controller
{
    ...
    {
        MultiImage::withTrashed()->where('product_id', $id)->restore();
    ...
    public function ProductImagePermanentDelete($id)
    {
        ...
        $multi_imgs = MultiImage::withTrashed()->where('product_id', $id)->get();
        foreach($multi_imgs as $multi_img){
            if(file_exists($multi_img->photo_name)){
                unlink($multi_img->photo_name);
            }
        }

        $proImg = Product::onlyTrashed()->findOrFail($id);
        MultiImage::withTrashed()->where('product_id', $id)->forceDelete();
